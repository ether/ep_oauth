...
`ep_oauth` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_oauth`:
...
* Server: `authenticate`, `authorize`, `expressConfigure`
