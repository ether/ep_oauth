# Contributing to ep_oauth

Thanks for helping improve `ep_oauth` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_oauth` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_oauth from your local clone
pnpm run plugins i --path ../ep_oauth

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_oauth/` are picked up after a server restart.
...
* Etherpad version, ep_oauth version, browser + OS
* Steps to reproduce
